...
describe("Enchanted Books CIT Generation", () => {
  ...
  describe("CIT Properties Generation", () => {
    it("should generate correct CIT properties for single-level enchantments", async () => {
      ...
      expect(citContent).toContain("model=assets/minecraft/models/item/enchanted_books/channeling_1");
    ...
    it("should generate correct CIT properties for multi-level enchantments", async () => {
      ...
      for (let level = 1; level <= 3; level++) {
        const citPath = join(outputDir, "assets", "minecraft", "optifine", "cit", `sharpness_${level}.properties`);
        expect(existsSync(citPath)).toBe(true);
        ...
        expect(citContent).toContain("type=item");
        expect(citContent).toContain("items=enchanted_book");
        expect(citContent).toContain(`model=assets/minecraft/models/item/enchanted_books/sharpness_${level}`);
  ...
  describe("Enchantment Model Generation", () => {
    it("should generate correct model structure for single-level enchantments", async () => {
      ...
      const modelPath = join(outputDir, "assets", "minecraft", "models", "item", "enchanted_books", "channeling_1.json");
      ...
      const hasHeldReference = model.overrides.some((o: any) => 
        o.model === "minecraft:item/books_3d/channeling_3d_open"
      );
      const hasOffhandReference = model.overrides.some((o: any) => 
    ...
    it("should generate correct model structure for multi-level enchantments", async () => {
      ...
      const modelPath = join(outputDir, "assets", "minecraft", "models", "item", "enchanted_books", "sharpness_2.json");
      expect(existsSync(modelPath)).toBe(true);
    ...
    it("should handle curse enchantment texture name mapping", async () => {
      ...
      const modelPath = join(outputDir, "assets", "minecraft", "models", "item", "enchanted_books", "binding_curse_1.json");
      expect(existsSync(modelPath)).toBe(true);
      ...
      const hasHeldReference = model.overrides.some((o: any) => 
        o.model === "minecraft:item/books_3d/binding_curse_3d_open"
      );
      const hasOffhandReference = model.overrides.some((o: any) => 
        o.model === "minecraft:item/books_3d/binding_curse_3d"
      );

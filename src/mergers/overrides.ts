import type { RequestMerger, WriteRequest } from '../file-manager';
...
export class OverridesMerger implements RequestMerger {
  ...
  merge(requests: WriteRequest[]): WriteRequest {
    ...
    console.log(`🔄 Merged ${requests.length} pommel-model requests into 1 (${uniqueOverrides.length} overrides, priority ${highestPriority})`);

...
describe("Books Resource Pack Regression Tests", () => {
  ...
  describe("Enchanted Books CIT/Pommel Pattern", () => {
    it("should generate correct CIT + Pommel pattern for enchanted books", async () => {
      ...
      expect(citChanneling).toContain(
      ...
      const channelingModelPath = join(
        outputDir,
        "assets",
        "minecraft",
        "models",
      ...
      const hasHeldReference = channelingModel.overrides.some(
        (o: any) => o.model === "minecraft:item/books_3d/channeling_3d_open"
      );
      const hasOffhandReference = channelingModel.overrides.some(
    ...
    it("should handle multi-level enchantments with level suffix in texture names", async () => {
      ...
      expect(citContent).toContain(
        "model=assets/minecraft/models/item/enchanted_books/sharpness_3"
      );
      ...
      expect(
        sharpness3Model.overrides.some(
  ...
  describe("3D Model Integration", () => {
    ...
    it("should correctly reference 3D models in Pommel overrides", async () => {
      ...
      if (existsSync(channelingModelPath)) {
        ...
        const hasOpenReference = channelingModel.overrides.some(
          (o: any) => o.model === "minecraft:item/books_3d/channeling_3d_open"
        );
  ...
  describe("Asset Preservation", () => {
    ...
    it("should preserve minecraft assets except items directory", async () => {
      ...
      await mkdir(join(inputDir, "assets", "minecraft", "blockstates"), {
        recursive: true,
      });

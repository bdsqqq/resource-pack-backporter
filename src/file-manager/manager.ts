...
import type { WriteRequest, FileManager } from './index';
import { getMergers } from '../mergers';
...
export class FileManagerImpl implements FileManager {
  ...
  private async mergeRequests(requestsByPath: Map<string, WriteRequest[]>): Promise<WriteRequest[]> {
    ...
    for (const [key, requests] of requestsByPath) {
      ...
      if (merger) {
        console.log(`🔄 Merging ${requests.length} requests for ${key} using ${merger.name}`);
        const merged = merger.merge(requests);
        mergedRequests.push(merged);
      } else {
        // No merger found, use highest priority request
        console.warn(`⚠️  No merger for ${key}, using highest priority request`);
  ...
  private async writeRequest(request: WriteRequest): Promise<void> {
    ...
    try {
      ...
      console.error(`❌ Failed to write ${request.type}: ${request.path}`, error);
      throw error;
    }
  }

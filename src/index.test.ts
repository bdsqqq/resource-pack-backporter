...
test("backports pack with display context switching", async () => {
  ...
  const baseTexture = await readFile(join(TEST_OUTPUT_DIR, "assets/minecraft/textures/item/test_sword.png"), "utf-8");
...
test("backports pack with no special components (base item)", async () => {
  ...
  const model = await readJsonFile(join(TEST_OUTPUT_DIR, "assets/minecraft/models/item/simple_item.json"));

...
import type { FileWriter, WriteRequest } from '../file-manager';
...
export class TextureCopyWriter implements FileWriter {
  ...
  async write(request: WriteRequest, outputDir: string): Promise<void> {
    ...
    if (!sourceContent.sourcePath) {
      console.error(`❌ No source path provided for texture copy: ${request.path}`);
      return;
    }
    ...
    try {
      ...
      console.error(`❌ Failed to copy texture ${request.path}: ${error.message}`);
    }
  }
}

...
import { ModelCompatibilityProcessor } from "../src/postprocessors/model-compatibility";
...
describe("Template File Protection", () => {
  ...
  describe("Template File Processing Protection", () => {
    it("should never add parent field to template files", async () => {
      ...
      await mkdir(join(testDir, "assets", "minecraft", "models", "item", "books_3d"), { recursive: true });
      const templatePath = join(testDir, "assets", "minecraft", "models", "item", "books_3d", "template_book_open.json");
    ...
    it("should validate template file structure after processing", async () => {
      ...
      await mkdir(join(testDir, "assets", "minecraft", "models", "item", "books_3d"), { recursive: true });
      const templatePath = join(testDir, "assets", "minecraft", "models", "item", "books_3d", "template_book_closed.json");
  ...
  describe("Edge Cases and Error Handling", () => {
    it("should reject malformed template files and preserve them unchanged", async () => {
      ...
      await mkdir(join(testDir, "assets", "minecraft", "models", "item", "books_3d"), { recursive: true });
      const templatePath = join(testDir, "assets", "minecraft", "models", "item", "books_3d", "template_malformed.json");
    ...
    it("should reject template files with missing elements array", async () => {
      ...
      await mkdir(join(testDir, "assets", "minecraft", "models", "item", "books_3d"), { recursive: true });
      const templatePath = join(testDir, "assets", "minecraft", "models", "item", "books_3d", "template_no_elements.json");
    ...
    it("should reject completely invalid template files", async () => {
      ...
      await mkdir(join(testDir, "assets", "minecraft", "models", "item", "books_3d"), { recursive: true });
      const templatePath = join(testDir, "assets", "minecraft", "models", "item", "books_3d", "template_empty.json");
  ...
  describe("Integration with Compatibility Fixes", () => {
    it("should apply zero-thickness fixes to templates without adding parent", async () => {
      ...
      await mkdir(join(testDir, "assets", "minecraft", "models", "item", "books_3d"), { recursive: true });
      const templatePath = join(testDir, "assets", "minecraft", "models", "item", "books_3d", "template_multi_issues.json");

...
export class WritableBookContentHandler implements ItemHandler {
  ...
  process(jsonNode: any, context: ProcessingContext): WriteRequest[] {
    ...
    if (Object.keys(contextMappings).length === 0) {
      console.log(`⚠️  No context mappings found for writable book ${context.itemId}`);
      return [];
    }
  ...
  private findWritableBookContentSelector(obj: any): any {
    ...
    if (obj.component === "minecraft:writable_book_content" || 
  ...
  private extractContextMappings(jsonNode: any, context: ProcessingContext): { [context: string]: string } {
    ...
    if (displayContextSelector) {
      ...
      if (displayContextSelector.fallback) {
        ...
        if (fallbackModel) {
          const standardContexts = ["gui", "fixed", "ground", "firstperson_righthand", "thirdperson_righthand", "firstperson_lefthand", "thirdperson_lefthand", "head"];
  ...
  private extractModelFromBookContent(modelObj: any): string | null {
    ...
    if (modelObj.type === "minecraft:condition" && modelObj.predicate === "minecraft:writable_book_content") {
  ...
  private extractTexture(contextMappings: { [context: string]: string }, context: ProcessingContext): string {
      ...
      try {
        ...
        if (found) {
          ...
          if (modelContent.textures?.layer0) {
            console.log(`📚 Extracted book texture: ${modelContent.textures.layer0}`);
            return modelContent.textures.layer0;
          }
        }
      } catch (error) {
        console.log(`⚠️  Error reading book model for texture: ${error.message}`);
      }
    }
  ...
  private buildWritableBookPommelModel(contextMappings: { [context: string]: string }, texture: string): any {
    const overrides = [];

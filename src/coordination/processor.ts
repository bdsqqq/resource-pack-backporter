...
import { ResourcePackIntrospector } from '../introspection';
import { FileManagerImpl } from '../file-manager';
import { getHandlers } from '../handlers';
import { getWriters } from '../writers';
import type { ProcessingContext } from '../file-manager';
...
export class BackportCoordinator {
  ...
  async backport(inputDir: string, outputDir: string, options: Partial<BackportOptions> = {}): Promise<void> {
    ...
    console.log(`🎨 Found ${this.packStructure.textureFiles.length} texture files`);
  ...
  private async processItemFile(
    ...
    if (this.verbose) {
      console.log(`  Components: ${analysis.componentsUsed.join(", ") || "none"}`);
      console.log(`  Contexts: ${analysis.displayContexts.join(", ") || "none"}`);
    }
